...
def detect(path: Path, original_filename: str | None = None) -> str:
    ...
    if head.startswith(b"\x7fELF"):
        return "elf"
    if head.startswith(b"UF2\n"):
        return "uf2"
    if head.startswith(b":") and (path.suffix.lower() == ".hex"):
        return "intel_hex"
    suffix = Path(original_filename or path.name).suffix.lower()
    suffix_types = {
        ".asar": "asar", ".js": "javascript", ".mjs": "javascript", ".cjs": "javascript", ".ts": "typescript",
        ".json": "json", ".json5": "json", ".inf": "inf", ".uf2": "uf2", ".bin": "firmware_binary",
        ".exe": "pe", ".dll": "pe", ".zip": "zip", ".7z": "7z", ".rar": "rar", ".cab": "cab", ".msi": "msi_or_ole",
        ".hex": "intel_hex", ".dmg": "dmg", ".pkg": "pkg", ".app": "macos_app",
    }
