...
`schema = "peripheral.opcode-acl/1"`. TOML rather than the YAML used by
...
| Class | Meaning | Generated? |
|---|---|---|
| `safe_read` | reads state, changes nothing | yes |
| `probe_ok` | may be sent to an unidentified board to identify it | yes |
| `safe_write` | writes volatile state | yes |
| `slow_flash` | writes state that survives a power cycle | yes |
| `destructive` | erases, resets, or enters a bootloader | **never** |
| `unknown` | not classified, or classified on insufficient evidence | **never** |
...
3. **`safe_read` and `probe_ok` require `hardware`, `hardware_third_party` or
   `firmware`.** Vendor JavaScript and assumption do not qualify.
4. **Every class a family uses must have measured timing**, and a `slow_flash`
...
5. **No duplicate opcode and no duplicate name within a family.**
6. Timing is per family. There is no global default and no fallback: an unknown
