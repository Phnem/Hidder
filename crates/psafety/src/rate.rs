...
impl RateLimiter {
    ...
    /// Whether this family and class have timing measured on hardware.
    pub fn is_measured(family: &str, class: OpcodeClass) -> bool {
        timing_for(family, class).is_some()
    }

    /// May an operation of this class go to this device now?
    ///
    ...
    pub fn check(
        ...
        let Some(timing) = timing_for(family, class) else {
    ...
    pub fn record(&mut self, device: DeviceId, family: &str, class: OpcodeClass, finished_ms: u64) {
        let state = self.devices.entry(device).or_default();
        state.last_finished_ms = Some(finished_ms);
        state.recent.push(finished_ms);
        if let Some(timing) = timing_for(family, class) {
...
mod tests {
    ...
    fn the_first_operation_goes_immediately() {
        ...
        assert_eq!(
            limiter.check(DEVICE(), KNOWN, OpcodeClass::SafeRead, 0, None),
            RateDecision::Allow
        );
    }
    ...
    fn a_second_operation_waits_the_families_measured_gap() {
        ...
        limiter.record(DEVICE(), KNOWN, OpcodeClass::SafeRead, 1_000);

        // royuan-gen2 declares 12 ms between reads, measured on hardware.
        assert_eq!(
            limiter.check(DEVICE(), KNOWN, OpcodeClass::SafeRead, 1_005, None),
            RateDecision::Wait {
                ms: 7,
                reason: WaitReason::MinGapBefore
            }
        ...
        assert_eq!(
            limiter.check(DEVICE(), KNOWN, OpcodeClass::SafeRead, 1_012, None),
            RateDecision::Allow
        );
    }
    ...
    fn the_quiet_after_a_flash_write_applies_to_everything_that_follows() {
        ...
        let mut limiter = RateLimiter::new();
        limiter.record(DEVICE(), KNOWN, OpcodeClass::SlowFlash, 0);

        match limiter.check(DEVICE(), KNOWN, OpcodeClass::SafeRead, 500, None) {
        ...
        assert_eq!(
            limiter.check(DEVICE(), KNOWN, OpcodeClass::SafeRead, 2_000, None),
            RateDecision::Allow
        );
    }
    ...
    fn the_burst_allowance_is_the_measured_one() {
        ...
        let mut limiter = RateLimiter::new();
        limiter.record(DEVICE(), KNOWN, OpcodeClass::SlowFlash, 0);
        limiter.record(DEVICE(), KNOWN, OpcodeClass::SlowFlash, 3_000);

        match limiter.check(DEVICE(), KNOWN, OpcodeClass::SlowFlash, 6_000, None) {
        ...
        assert_eq!(
            limiter.check(DEVICE(), KNOWN, OpcodeClass::SlowFlash, 10_001, None),
            RateDecision::Allow,
    ...
    fn a_failed_operation_still_owes_its_quiet_period() {
        ...
        limiter.record(DEVICE(), KNOWN, OpcodeClass::SlowFlash, 0);
        assert!(matches!(
            limiter.check(DEVICE(), KNOWN, OpcodeClass::SafeRead, 100, None),
            RateDecision::Wait { .. }
        ));
    }
    ...
    fn an_unknown_family_is_never_given_a_default_interval() {
        let limiter = RateLimiter::new();
        assert_eq!(
            limiter.check(DEVICE(), UNKNOWN, OpcodeClass::SafeWrite, 0, None),
            RateDecision::NeedsConfirmation,
            "an unmeasured family must ask, not guess"
        );
        assert!(
            !RateLimiter::is_measured(UNKNOWN, OpcodeClass::SafeWrite),
    ...
    fn an_unknown_family_needs_a_fresh_confirmation_for_every_operation() {
        ...
        limiter.record(DEVICE(), UNKNOWN, OpcodeClass::SafeWrite, 10);

        // No amount of elapsed time turns the next one into an automatic
        // operation. The confirmation was spent on the first.
        assert_eq!(
            limiter.check(DEVICE(), UNKNOWN, OpcodeClass::SafeWrite, 10_000_000, None),
            RateDecision::NeedsConfirmation,
    ...
    fn a_known_family_with_no_measurement_for_a_class_is_treated_as_unknown() {
        ...
        assert!(!RateLimiter::is_measured(KNOWN, OpcodeClass::SafeWrite));
        let limiter = RateLimiter::new();
        assert_eq!(
            limiter.check(DEVICE(), KNOWN, OpcodeClass::SafeWrite, 0, None),
            RateDecision::NeedsConfirmation
        );
    }
    ...
    fn two_devices_do_not_share_a_cooldown() {
        ...
        limiter.record(DEVICE(), KNOWN, OpcodeClass::SlowFlash, 0);
        assert_eq!(
            limiter.check(OTHER_DEVICE(), KNOWN, OpcodeClass::SafeRead, 1, None),
            RateDecision::Allow,
    ...
    fn one_familys_numbers_are_not_applied_to_another() {
        ...
        let gen2 = timing_for("royuan-gen2", OpcodeClass::SafeRead).expect("measured");
        let yc500 = timing_for("royuan-yc500", OpcodeClass::SafeRead).expect("measured");
        assert_eq!(gen2.family, "royuan-gen2");
        assert_eq!(yc500.family, "royuan-yc500");
        assert!(
            timing_for("aula-hero84-he", OpcodeClass::SafeRead).is_none(),
    ...
    fn forgetting_a_device_clears_its_cadence() {
        ...
        assert_eq!(
            limiter.check(DEVICE(), KNOWN, OpcodeClass::SafeRead, 1, None),
            RateDecision::Allow
        );
    }
}
