"""HID device enumeration and candidate discovery for Windows (English edition).
...
IGNORE_KEYWORDS = [
    "bluetooth", "audio", "sound", "headset", "microphone", "speaker",
    "tp-link", "wi-fi", "wireless adapter", "hub", "concentrator",
    ...
    try:
        ...
        if proc.returncode == 0 and proc.stdout:
            ...
            for it in items:
                ...
                cat = "generic_hid"
                if pnp_class == "Keyboard" or "keyboard" in friendly.lower() or "клавиатура" in friendly.lower():
                    cat = "keyboard"
                elif pnp_class == "Mouse" or "mouse" in friendly.lower() or "мышь" in friendly.lower():
                    cat = "mouse"
...
def _mock_hid_candidates(category_filter: str | None = None) -> list[DiscoveredHidCandidate]:
    all_mocks = [
        DiscoveredHidCandidate(
            display_name="Epomaker / Skyloong / AULA — Keyboard",
            vid="0x372E",
            pid="0x103E",
