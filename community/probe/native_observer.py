"""Native Windows desktop application observer using transparent MinHook DLL injection.
...
class NativeVendorObserver:
    ...
    def _ensure_helper_dll(self) -> Path | None:
        ...
        # 1. Check if helper is already present in source tree or bundled asset
        module_dir = Path(__file__).resolve().parent
        candidate_sources = [
            module_dir / "assets" / HELPER_DLL_NAME,
            module_dir / "assets" / "probe_hook_x64.dll",
            module_dir.parent / "probe_hook" / "target" / "release" / "probe_hook.dll",
        ]
        ...
        # 2. Check or deploy to stable runtime path
        ...
        try:
            ...
            # Fallback to direct path if copy fails
            return src_dll
