...
class PassiveTransportObserver:
    ...
    def _handle_raw_event(self, ev: dict[str, Any]) -> None:
        ...
        # Privacy Guard against Keylogging:
        # If input report is a standard boot keyboard report outside active guided input tests, drop it
        if api == "inputreport" or direction == "in":
            if self.active_action_id not in (
                ...
                # Check for standard 8-byte boot keyboard report
    ...
    def record_event(
        ...
        # Deduplication & JSON Compaction: update repeat_count and last_seen
    ...
    def correlate_actions(
        ...
        """Correlate actions with pairwise A -> B -> A change/restore comparison and structural matching."""
        ...
        for base_id, pair in pairs.items():
            ...
            # Filter out idle background polling noise if novel events exist
            ...
            # Structural grouping: match change packet with corresponding restore packet
            # Group by (report_id, length, opcode_prefix)
            def _struct_key(hex_str: str) -> tuple[int, int, str]:
                # first 2 bytes (4 hex chars) represent opcode / command family
            ...
            # Find matching real baseline packets (NO fake zero packets)
            ...
            # Compare change (B) vs restore (A')
            for ch_hex in change_reports:
                ch_key = _struct_key(ch_hex)
                # Find matching restore packet with same structural key
                ...
                if matching_rst and len(ch_hex) == len(matching_rst):
                    ...
                    # Classify checksum vs semantic fields
                    # If highest diff offset is at the command tail (e.g. byte 7 in 8-byte frame or byte 63 in 64-byte frame)
            ...
            # Determine restore_matches_original
            ...
            # RT Press & Release table disambiguation
            notes = None
        ...
        for action in single_actions:
            ...
            # Match against real baseline if available
            baseline_reports = []
            for b_ev in self.idle_baseline_events:
                b_hex = b_ev["bytes_hex"]
