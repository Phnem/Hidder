...
# Shared lower-level primitives for the register-0x01 global-lighting path.
# Single source of truth consumed by BOTH this diagnostic probe AND the normal
# executor transport (aula_transport.light.brightness), so they cannot drift.
from .lighting_core import (  # noqa: E402
    LIGHT_MODE_REG, BRIGHTNESS_OFFSET, BRIGHTNESS_MIN, BRIGHTNESS_MAX,
    REGISTER_WIDTH,
    plan_brightness_temporary, is_valid_baseline,
    canonical_set_frame as _canonical_set_frame,
    normalize_echo, decode_echo, verify_echo,
    _final_verified, _rollback,
    read_light_state, build_light_state_frame, set_light_state_with_echo,
)
...
def _real_session_factory() -> callable:
    def make():
        ...
        def get_light() -> bytes:
            return read_light_state(raw, p)

        def set_light(reg7: bytes) -> bytes:
            return set_light_state_with_echo(raw, reg7)
