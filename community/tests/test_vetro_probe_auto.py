...
def test_scenario_a_known_device_complete(tmp_path):
    ...
    # 6 evidence-gated AUTO_REVERSIBLE ops (profile, polling, win_lock, deadzone,
    # actuation, brightness) x (write + rollback); remap/rt stay BLOCKED by
    # feature-level evidence gates (E5 open, units crosscheck open).
    assert trans.device.write_count == 12
    pkg = run.package_dir
