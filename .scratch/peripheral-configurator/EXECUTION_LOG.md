...
## 2026-08-19 — TICKET-13 (Tauri UI: Devices + HE + Journal)

### Что закрыто

Первая точка, где результат фазы 1 виден не разработчику в консоли, а человеку
в окне: подключить плату → увидеть карточку → нажать Connect → прочитать
actuation → увидеть журнал. На реальном железе, без мок-данных на любом экране.

Экранов четыре, а не три: добавлен About, потому что сборка читает четыре
клавиши на одной плате и не пишет ничего, и это человек должен узнать из
интерфейса, а не вывести из него.

### Три слоя, которых не было

1. **`pcore::service`** — один поток владеет `Peripheral`, `DeviceService` —
   `Send + Sync` ручка, которая умеет только просить. Альтернативой был
   `Mutex<Peripheral>`, то есть ровно форма, запрещённая FR10. Это **не**
   `DeviceSession` из FR10 (та владеет handle'ами всю жизнь сессии и строится в
   TICKET-15 под аналоговый стрим) — та же форма слоем выше.
2. **Watcher**, и вместе с ним граница, которую он обязан держать:
   *enumeration* (вопрос к ОС, ничего не открывает) крутится раз в 2 секунды;
   *identification* (чтение дескрипторов, handle на коллекцию) — один раз на
   устройство. Watcher, не разделявший бы их, открывал бы каждую HID-коллекцию
   на машине каждый тик.
3. **Трёхканальный IPC целиком**: шесть команд, четыре события (+ объявленный
   `device:battery-changed`, который никто не испускает), и Channels
   по-прежнему зарезервированы и отказывают — плюс тест, что аналоговый поток не
   подписан через шину событий.

### Находка уровнем ниже UI: перечисление отвечало из кэша

`hidapi` строит список устройств один раз, при открытии контекста. Без
`refresh_devices()` выдернутое устройство числилось бы подключённым **навсегда**,
а подключённое позже не появлялось бы никогда — то есть acceptance criteria про
отключение было недостижимо, и причина лежала в `ptransport`, а не в UI.
Добавлен `Hid::refresh`.

Стоит записи как класс: **тикет на UI обнаружил дефект транспорта**, потому что
UI — первый потребитель, которому важно, что состояние меняется во времени.

### Две правки прошлых тикетов, сделанные здесь по необходимости

- **Журнал переехал из сессии к вызывающему.** `connect()` создавал `JournalLog`
  внутри и отдавал внутри `DeviceSession`; записи **неудавшегося** подключения
  умирали вместе с сессией, которой не случилось. Терялось ровно то, ради чего
  журнал существует. Теперь один журнал на процесс, отказы и ошибки в нём
  наравне с успехами.
- **Одни часы на процесс.** Гейт строится на операцию, `MonotonicClock` обнулялся
  при создании, значит каждая запись получала время около нуля. `Copy` +
  `Peripheral` держит одни часы. Видно: `+41089 мс` / `+100576 мс` вместо двух
  нулей.

### Два дефекта, найденных живым окном и исправленных

1. Кнопка Connect висела на устройствах без config-канала — на обычной машине
   это большинство HID-устройств, и ряд серых кнопок читается как сломанное
   приложение. Кнопки теперь нет вовсе; у залипшего устройства — наоборот, есть
   и задизейблена, потому что отсутствие действия там нужно объяснить.
2. **Connect ухудшал структурную ось.** `connect()` из TICKET-12 пересчитывал
   ...
   падал с `candidate` до `unknown`. Обмен не может изменить форму устройства.

### Дефект, найденный и сознательно НЕ исправленный

**Байтовые размеры отчётов в реестре разъехались с транспортом ровно на
единицу.** Структурная ось на всех трёх собственных устройствах проекта отдаёт
`candidate` вместо `verified`.

`ptransport::shape` считает размеры **включая байт report id** (осознанно,
закреплено его тестами); `data/devices/*.toml` хранят объявленный дескриптором
payload **без** него. Каждая коллекция с report id расходится на единицу;
единственная коллекция AULA без report id совпадает точно. Все семь
`descriptor_fnv1a64` совпадают — дескрипторы не менялись.

Почему не поймали: `pregistry/tests/real_hardware.rs` гоняет захваченный JSON
через тот же путь, что и данные реестра, поэтому обе стороны согласны между
собой, а расхождение с живым железом невидимо. **Это ровно та ловушка, о которой
предупреждает запись TICKET-10** — регрессионный тест начал соглашаться с кодом
вместо железа. Предупреждение было записано и всё равно сработало.

Не исправлено внутри UI-тикета сознательно: выбор канонической величины —
решение уровня `pregistry`, генератор этих TOML не закоммичен в `scripts/`
вообще (что само по себе нарушает test seam «сгенерированные артефакты
перегенерируются в CI»), а правка fingerprint-данных из UI-тикета — дрейф scope.
Безопасности не касается: продуктовая ось по-прежнему `verified`, запись
гейтится по оси семейства.

### Prior art

`docs/prior-art/ui-prior-art.md` — Pipette Desktop, OpenRGB, libratbag/Piper,
Vial/Keychron Vial/QMK. Все copyleft, все в режиме `docs`, кода не взято.
Pipette добавлена в `inventory.md`.

Полезнее всего оказалось не то, что у них есть, а то, чего нет: **ни один из
четырёх не показывает, откуда известно, что число означает то, что означает.**
Им и не нужно — протокол либо открыт, либо получен от производителя. У нас он
восстановлен и проверен на одной плате, поэтому `origin` / `confidence` / имя
команды / provenance рядом со значением — это и есть то, чем наш экран HE обязан
от них отличаться.

### Правило, добавленное по ходу

**Значение не переживает сессию, которая его прочитала.** Число, оставшееся на
экране после того как устройство выдернули или его endpoint залип, — экран,
тихо сообщающий состояние железа, которое сейчас ничем не подтверждено. В тикете
этого не было; проверено четырьмя тестами и на железе.

Родственное: `valuesByLabel` **отказывается** привязать значение к метке,
которая встречается в раскладке дважды (`Shift`, `Ctrl`, `Alt`). Иначе одно
измерение печаталось бы под клавишей, которую никто не мерил — тот же класс
ошибки, за который проект уже заплатил обменом 003.

### Verification

`fmt`, `clippy --workspace --all-targets -D warnings`, `test --workspace`,
`check_crate_dag.py`, `npm run typecheck`, `npm test` (36), `npx vite build`,
`cargo run -p pcore --example read_actuation` (на железе), `npx tauri dev` —
все зелёные. Rust-тестов `+21`, фронтенд-тестов `+36` (их не было).

`cargo deny check advisories` не зелёный — GTK3 через Tauri, как и до тикета.

### Не проверено

CI (remote-репозитория нет), физическое выдёргивание кабеля (нет прав
администратора, а отключать пользовательскую клавиатуру без просьбы неуместно;
логика диффа watcher'а вынесена в чистую функцию и покрыта тестами), stall
(ни разу не случился, путь проверен только синтетикой).

### Next eligible ticket

TICKET-15 (verified write + Analog Monitor) — фаза 3 закрыта целиком.
