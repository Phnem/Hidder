...
**Уточнение 2026-08-17 (remote-валидация EPOMAKER).** Этот эпик **не форсируется** ради удалённого теста EPOMAKER. Если продуктовому пути раньше срока понадобится маленький переиспользуемый компонент `Export Device Report`, он выделяется отдельным узким тикетом или как ранняя часть Inventory Level 0 — и обязан быть: read-only, без safe probes, без write, без capture пользовательского ввода, с serial redaction по умолчанию, с versioned artifact schema. Расширять этот узкий компонент в полноценный TICKET-16 раньше времени нельзя. Уровень 2 (Safe Probe) в remote-сценарии не участвует вообще.
...
## Архитектурное требование, добавлено 2026-08-18 (TICKET-12): что такое UserConfirmation

TICKET-12 ввёл `bootstrap_probe` и одноразовый `ProbeGate`. Уровень 2 (Safe Probe) этого эпика — прямой потребитель того же механизма, поэтому семантика подтверждения фиксируется здесь заранее.

`UserConfirmation` — это UX/safety capability, свидетельство что человека спросили про **эту одну** операцию. Не секретная строка, не флаг, не конфиг.

- production UI **не** минтит confirmation при инициализации, при открытии экрана или «на всякий случай»;
- background task не может подтвердить probe вообще никогда;
- одно явное действие пользователя = одна probe authorization = один `ProbeGate` (который потребляется отправкой);
- путь: явное действие в UI → `ConfirmationReceipt` → одноразовый `ProbeGate` → один типизированный read.

В dev-инструментах допустим отдельный явный dev-only механизм (сейчас `--confirm-bootstrap-probe` в `pproto/examples/aula_probe.rs`). Он **не** является образцом для продукта, и это записано в докстринге самого файла.
