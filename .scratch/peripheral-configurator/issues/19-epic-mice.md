...
Будет детализирован при декомпозиции. **Обновлено 2026-08-18:** одна мышь уже есть локально — VXE Dragonfly R1 SE+ с 2.4 ГГц ресивером, и её read-only инвентарь снимается в TICKET-22 задолго до этой фазы. Это меняет входные данные эпика, но **не переносит его работу вперёд**: TICKET-22 даёт identity и topology, а `MouseProtocol`, DPI/polling/кнопки и любые записи остаются здесь. Понадобится ли дополнительно Sinowealth-мышь, решится по данным TICKET-22 и по тому, какое семейство окажется у имеющейся.
