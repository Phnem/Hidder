...
Три блокера, и все три — софтверные:

1. **BY wired `0x04` discriminator** — нужен BY-профиль и синтетический
   `getBattery` из схемы `hpe`.
2. **CZ config payload semantics** — конфигуратор не рендерится на нулях; нужны
   вендорские defaults (`default-keys-god-60.js`) или парсеры из несобранного
   `/cizhou/_next/`. Без этого нет ни UI-инвентаря, ни sweep'ов, ни layout.
3. **Полный UI-инвентарь и sweep'ы** — следуют из (2).
