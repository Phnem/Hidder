...
Аудит эха и `EVIDENCE_VOID` на реальных ответах, инвентарь UI-действий с
непустым списком «найдено, не выполнено», sweep'ы ≥4 точек, stride по одной
записи, layout-lane, `hardware_shaped_hole`. Всё это по-прежнему обязательно
для A Preview.
