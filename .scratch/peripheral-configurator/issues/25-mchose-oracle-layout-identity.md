...
1. **BY wired `0x04` discriminator.** `setPerformance` снят (10 кадров, поля
   локализованы). `setReset` — нет. Нужно довести UI до того, чтобы диалог сброса
   открывался при корректных по длине ответах: в прогонах с короткими ответами он
   открывался всегда, с правильными — не открылся ни разу. Это вопрос
   харнесса/UI, не протокола, и железа он не требует.
