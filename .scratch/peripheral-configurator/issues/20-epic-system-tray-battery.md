...
**Обновлено 2026-08-18.** Battery-трек был заблокирован формулировкой «ни одно доступное устройство не беспроводное». Это больше не так: VXE Dragonfly R1 SE+ подключается через 2.4 ГГц ресивер и находится у разработчика. Блокировка при этом **не снимается автоматически** — беспроводное подключение не означает читаемый заряд. TICKET-22 наблюдает, есть ли стандартный battery HID usage; вендорский путь (собственный опкод, запрос к ресиверу) потребовал бы probe и лежит вне инвентаря. Если найдётся очевидный безопасный путь, он оформляется отдельным read-only тикетом, и уже он разблокирует этот трек.
