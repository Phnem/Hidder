...
Current ticket: None
Last completed ticket: TICKET-13 (Tauri UI — Devices + HE + Journal, read-only, на реальном железе)
...
Разбивка 22 тикетов на исполнительные фазы и два параллельных трека. Фазы — про порядок работы в этой сессии/сессиях; нумерация НЕ совпадает с «Phase 0..6» исходного плана (там фазы продукта). Соответствие указано в колонке «План».
...
| **3. Read-only вертикальный срез** — **ЗАКРЫТА 2026-08-19** | `psafety`-скелет, fingerprinting, первый protocol engine (AULA read-only), Tauri UI, эмулятор+CI-тест | **11 → 10 → 12 → 14 → 13, все DONE** | Phase 1 DoD (AC2) | 07, 08, 22 (все выполнены) |
...
### Phase 2 — Hardware Reality (перестроена трижды: дважды 2026-08-17, снова 2026-08-18)
...
**Третья перестройка (2026-08-18):** локально появилось второе физическое устройство, и это устройство **другой категории** — беспроводная мышь **VXE Dragonfly R1 SE+** с 2.4 ГГц ресивером. Фаза переоткрыта одним тикетом: TICKET-22 снимает её инвентарь во всех поддерживаемых режимах подключения. Фаза перестала быть «AULA Hardware Reality» и стала просто **Hardware Reality**.
...
### VXE Dragonfly R1 SE+ — второе локальное устройство (2026-08-18)

Беспроводная мышь с 2.4 ГГц ресивером, вариант `44857375424730` (официальная страница ATK). В отличие от EPOMAKER — **физически у разработчика**, поэтому исследуется обычным путём, инструментом TICKET-08.

Почему это вставлено **перед** TICKET-10, а не отложено до фазы 6: до сих пор в распоряжении реестра была ровно одна topology — проводная клавиатура на трёх USB-интерфейсах. `pregistry`, спроектированный против одного примера, почти наверняка окажется keyboard-specific там, где этого никто не заметит до фазы 6. Мышь плюс ресивер дают минимум три реальные topology и ставят перед реестром вопросы, которые на одной клавиатуре просто не возникают:
...
Дешевле ответить на них до того, как реестр написан.

**Заранее ничего не назначать.** Поддержка Bluetooth этим конкретным экземпляром — предмет наблюдения, а не вывод из названия модели. Наличие стандартного battery HID usage — тоже.

**Границы, которые тикет не переходит:** строго read-only (ни write, ни feature-чтения, ни input-репортов, ни probe); никакой VXE-специфичной ветки в `ptransport`; полный `MouseProtocol` и любые записи DPI/polling/кнопок остаются в фазе 6 и не переносятся. Если инвентарь покажет очевидный безопасный путь к заряду — это finding и предложение отдельного read-only тикета, а не расширение TICKET-22.
...
| **Хранилище реестра** (добавлено 2026-08-18, TICKET-10) | Сгенерированная Rust-таблица принята как временная. **До TICKET-18 / первых community submissions** обязателен выбор: сгенерированный SQLite по исходной архитектуре либо отдельный ADR с обоснованием смены решения. Не «когда-нибудь»: чужие данные без ревьюабельного хранилища — это то, ради чего Приложение B и писалось | `issues/10-*.md` § Correction, `spec.md` Приложение B |
...
- Референсное железо: **AULA Hero 84 HE** (проводная клавиатура) и **VXE Dragonfly R1 SE+** (беспроводная мышь с 2.4 ГГц ресивером, добавлена 2026-08-18) — оба на машине разработчика. Обновлено 2026-08-17 дважды: покупка ROYUAN-платы отменена; EPOMAKER находится у стороннего владельца и переведена в **remote external validation track** (не reference board). **Принадлежность EPOMAKER к какому-либо protocol family не предполагается заранее** — её устанавливают данные удалённого инвентаря.
...
| TICKET-12 | Первый protocol engine (AULA, read-only) | **DONE_WITH_DEVIATIONS** | TICKET-08/10/11 — все выполнены | `a84de24` | самопроверка, блокеров нет |
| TICKET-13 | Tauri UI skeleton (Devices/HE/Journal) | **DONE_WITH_DEVIATIONS** | TICKET-07/12 — выполнены | (в рабочем дереве) | самопроверка, блокеров нет |
| TICKET-14 | Device emulator + CI fingerprint-тест | **DONE_WITH_DEVIATIONS** | TICKET-08/10 — выполнены | `ce2fb2a` | самопроверка, блокеров нет |
| TICKET-15 | EPIC: Verified write + Analog Monitor | **READY — следующий** | TICKET-07..14 — все выполнены | — | — |
...
| TICKET-23 | MCHOSE — приобретение артефактов, провенанс, границы семей | **DONE_WITH_DEVIATIONS** | — | `3200a5a`, `2ceb4d8`, `124c140` | самопроверка; каталог оказался не в бандле |
| TICKET-24 | MCHOSE — STATIC LANE: кодек, читатель ответов, опасные пути | **READY — следующий** | TICKET-23 (done) | — | — |
| TICKET-25 | MCHOSE — ORACLE + LAYOUT + IDENTITY (живая семантика без железа) | PENDING | TICKET-23 (done), TICKET-24 (мягко) | — | — |
| TICKET-26 | Калибровочный гейт: починить движок инференса, сдать вслепую на AULA | **DONE — ГЕЙТ ЗЕЛЁНЫЙ** | — | `b1a0c16`, `389ea7e`, `ae3e090`, `f828d2b` | независимо перепроверено против §3.2 |
...
**Гейт-зависимость — СНЯТА 2026-08-24.** Playbook §3 делает калибровочный гейт на AULA обязательным перед первым signed bundle любой новой семьи. На момент открытия трека он был **красный и никогда не сдавался** (слепые режимы A/B: `HIGH_CONFIDENCE_WRONG` = 1 у v1 и 3 у v2 при требовании строго 0; `EXACT_PACKET_MATCH` 0/18 при ≥ 0.9).

TICKET-26 закрыл его в тот же день. Оба слепых режима, два независимо пре-регистрированных прогона: **HCW = 0, EXACT_PACKET_MATCH 18/18, BYTE 1.0, FIELD_OFFSET 1.00, ENDIANNESS 1.00, CHECKSUM yes**. Негативный контроль даёт RED (HCW 4) — гейт умеет падать, и это проверено, а не заявлено.

> **Запрет на A Preview / signed bundle для MCHOSE снят.** Оговорка §9 остаётся в силе и её нельзя потерять: гейт защищает от **регрессии конвейера**, он **не** доказывает корректность на незнакомой семье. Второй слепой таргет (v3 §62.2) по-прежнему обязателен, и мерить на нём надо долю узлов, дошедших до `VERIFIED_VIRTUAL` без человека, и число противоречий — а не «точность LLM».

**Открытый долг, обнаруженный попутно и предшествующий обоим тикетам:** `observations/transactions.jsonl` больше не воспроизводит замороженные датасеты — апстримный эмуляторный трейс вырос 2425→4324 кадров без пиннинга (нарушение §1.3 шага 1). Замороженные `benchmark/`-датасеты не пострадали, но провенанс промежуточного слоя надо чинить отдельным тикетом.
...
### TICKET-10 — `pregistry`: три оси идентичности

Status: DONE_WITH_DEVIATIONS (2026-08-18)
Tracker reference: локальный (`issues/10-multi-signal-fingerprinting.md`)
Dependencies: TICKET-07/08/11/22 (все done)
Acceptance criteria: все десять выполнены
Implementation summary: `data/devices/*.toml` (три реально наблюдавшихся endpoint) → `build.rs` → встроенный реестр. Матчер даёт **три независимых исхода** — structural / product / protocol-family, — у каждого свой confidence и список сработавших сигналов. `pcaps::Confidence` стал общим словарём для `pregistry` и `psafety`. `SafetyGate::identify_device` принимает family-confidence и отклоняет запись ниже `Verified` (`Refusal::UnverifiedFamily`), не трогая чтения.
...
12. **Второе локальное устройство: VXE Dragonfly R1 SE+, и оно исследуется до TICKET-10** (2026-08-18, пользователь). У разработчика появилась беспроводная мышь с 2.4 ГГц ресивером (вариант `44857375424730`, официальная страница ATK). Решение — не ждать фазы 6, а снять её read-only инвентарь немедленно, новым TICKET-22, **перед** TICKET-10. Основание: `pregistry` и fingerprinting, спроектированные против единственной topology (проводная клавиатура, 3 интерфейса, 7 TLC), почти неизбежно получатся keyboard-specific, и обнаружится это только в фазе 6. Мышь плюс ресивер дают минимум три реальные topology и ставят перед реестром вопросы, которых на одной клавиатуре не существует: одно ли это устройство в разных режимах подключения, одно ли устройство ресивер и мышь, какие сигналы переживают смену режима. Границы решения зафиксированы явно: тикет строго read-only (никаких write, feature-чтений, input-репортов и probe), VXE-специфичной ветки в `ptransport` не создаётся, полный `MouseProtocol` и записи DPI/polling/кнопок остаются в фазе 6. Находка о доступном battery state, если она случится, оформляется отдельным read-only тикетом, а не расширением TICKET-22.
...
| **Данные реестра и транспорт считают размеры отчётов по-разному** | TICKET-13 → отдельная задача | Новый, 2026-08-19 (TICKET-13). `ptransport::shape` считает размеры **включая байт report id** (осознанно, закреплено его тестами); `data/devices/*.toml` хранят объявленный дескриптором payload **без** него. Расхождение ровно на единицу у каждой коллекции с report id; коллекция без report id совпадает точно; все `descriptor_fnv1a64` совпадают. Следствие: структурная ось на всех трёх собственных устройствах отдаёт `candidate` вместо `verified`. **Почему не поймали:** `pregistry/tests/real_hardware.rs` гоняет захваченный JSON через тот же путь, что и данные реестра, поэтому обе стороны согласны между собой и расхождение с живым железом невидимо — та самая ловушка, о которой предупреждает лог TICKET-10, сработавшая несмотря на предупреждение. **Безопасности не касается** (продуктовая ось `verified`, запись гейтится по семейству). Требует: решить каноническую величину, перегенерировать TOML, **закоммитить генератор** (его нет в `scripts/` вообще — нарушение test seam «сгенерированные артефакты перегенерируются в CI»), и завести проверку, которая падает при деградации `verified` → `candidate`. |
...
| **Порядок силы fingerprint-сигналов из спецификации неполон** | ~~TICKET-10~~ **закрыт 2026-08-18** | Был новым в тот же день (TICKET-22). На VXE хеш дескриптора и набор TLC — сильнейшие сигналы по `spec.md` — идентичны у ресивера и у самой мыши, а различают их только три слабейших: product string, release, VID:PID. Реестр, взвешивающий сигналы по одному общему ранжированию, будет уверенно отвечать на вопрос «что это за устройство» и молча ошибаться на вопросе «с каким физическим предметом я разговариваю». **Закрыт TICKET-10:** линейное ranking отменено, сигналы разведены по трём осям на уровне типа, регрессия VXE wired vs receiver прогоняется на реальных захватах. Остаточная часть перешла в новую строку ниже. |
| ~~**Ось protocol-family пуста на всём реальном железе**~~ — **ЗАКРЫТ 2026-08-18 (TICKET-12), подтверждён в окне 2026-08-19 (TICKET-13)** | TICKET-12 | Семейство `aula-bytech` устанавливается обменом, `[verified]`, и путь «family определена → запись разрешена» впервые исполнился целиком: она разрешена осью семейства и всё равно невозможна, потому что write-команд в ACL ноль. Обе половины видны на экране Devices. Исходная формулировка: Новый, 2026-08-18 (TICKET-10). Правило «запись только при `confidence >= Verified` для family» реализовано и проверено, но **только на синтетических записях**: ни у AULA, ни у обоих VXE-endpoint семейство не установлено, потому что обмена ни с одним из них не было. Значит путь «family определена → запись разрешена» целиком не исполнялся ни разу. Первое реальное заполнение — TICKET-12, и именно там правило впервые что-то разрешит; до тех пор считать его проверенным нельзя. |
| Family достижима только через product | ~~TICKET-10~~ **исправлено 2026-08-18** | Был заложен в первую редакцию матчера: неизвестный SKU означал недостижимую family. Неверно — вендор отгружает ту же прошивку под новым product id, и незнакомый SKU не равен незнакомому протоколу. Добавлен независимый маршрут `ProtocolEvidence` (не capped продуктом), regression-тест «unknown product + verified protocol evidence → family identified, product Unknown». Structural match по-прежнему family не подтверждает. |
| Структурная ось доверяет чужим записям как своим | TICKET-18 | Новый, 2026-08-18 (TICKET-10). Точное совпадение структурного digest даёт `Verified` на том основании, что все записи реестра сняты с нашего железа. Как только появятся community-submissions, это основание исчезнет, а код останется. Структуре понадобится собственный evidence-маркер до первого приёма чужих данных. |
